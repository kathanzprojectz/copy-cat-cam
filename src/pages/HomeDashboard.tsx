...
const quickActions = [
  { icon: CalendarDays, title: "Meal Plan", desc: "View weekly schedule" },
  { icon: Package, title: "Inventory", desc: "Manage your fridge" },
  { icon: Sparkles, title: "AI Suggestions", desc: "Get recipe ideas" },
  { icon: BookOpen, title: "Recipes", desc: "Browse collection" },
];
...
const HomeDashboard = () => {
  ...
  return (
    <div className="app-container bg-background px-6 pt-10 pb-24 min-h-screen">
      {/* Header */}
      <p className="text-muted-foreground">{greeting}, Sarah! {greetEmoji}</p>
      ...
      <div className="bg-card rounded-3xl p-5 shadow-sm mb-6 animate-fade-in">
        ...
        <div className="flex gap-4">
          <CalorieRing current={1250} total={2000} />
          <div className="flex-1 space-y-3">
            {meals.map((m) => (
              <div key={m.name}>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-muted-foreground">{m.name}</span>
                  <span className="font-medium text-foreground">{m.cal} cal</span>
                </div>
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-primary">750 cal</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card rounded-3xl overflow-hidden shadow-sm mb-6 animate-fade-in">
        ...
        <div className="p-5">
          ...
          <div className="flex gap-2 mb-4">
            <span className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-full bg-card border border-primary/30 text-primary font-medium">
          ...
          <button className="w-full py-3.5 rounded-2xl bg-primary text-primary-foreground font-semibold shadow-lg shadow-primary/25 active:scale-[0.98] transition-transform">
            View Recipe
          </button>
        </div>
      </div>
      ...
      <div className="bg-card rounded-3xl p-5 shadow-sm mb-6 animate-fade-in">
        ...
        <button className="w-full py-3.5 rounded-2xl border border-border font-semibold text-foreground active:scale-[0.98] transition-transform">
          Cast Your Vote
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        {quickActions.map((a) => (
          <div key={a.title} className="bg-secondary rounded-2xl p-5">
